...
public class JsonBinder {
	 ...
	 */
	public static JsonBinder nonEmptyMapper() {
	 ...
	 */
	public static JsonBinder nonDefaultMapper() {
	...
	public <T> Map<Long,String>  getObjectsFromJson(String jsonString){ 
        try{
        	JsonNode nodes = mapper.readTree(jsonString); 
        	Map<Long,String> map  = new HashMap<Long,String>(); 
	        for(JsonNode node : nodes){ 
	        	map.put(Long.parseLong(node.get("uid").textValue()), node.toString()); 
	        }
	        return map;
        } catch (IOException e) {
			logger.warn("parse json string error:" + jsonString, e);
