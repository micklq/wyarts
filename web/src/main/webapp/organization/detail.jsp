...
<title>权限信息维护</title>
<%  
  Organization p = (Organization) request.getAttribute("organization");
  if(p==null){ p = new Organization();} 
%>
</head>
<body>
<article class="cl pd-20">
<form method="post" class="form form-horizontal" id="form-submit">
   <input type="hidden" id="id" name="id" value="<%=p.getOrgId()%>">
            <div class="row cl">
           ...
           <% for( OrganizationStatus o : OrganizationStatus.values() ){ %>
            <option value="<%= o.getValue() %>" <%=(p.getStatus()== o.getValue() ? "selected" : "")%>><%= o.getName() %></option>
           <% } %>
          </select>
          </span>
          </div>
        ...
        $(function () { 
            $("#form-submit").validate({
                ...
                submitHandler: function (form) {
                    $(form).ajaxSubmit({
                        type: 'post',
                        url: "<%=basePath%>/organization/saveAction",
